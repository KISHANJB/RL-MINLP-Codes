...
for vec in generate_valid_vectors(N):
    ...
    for i, var_name in enumerate(int_var_names):
        ...
        constr_lower = f"constraintL{i + 2}"
        constr_upper = f"constraintU{i + 2}"
        lb = vec[i - 1]
        ub = vec[i + N - 1]
    
            # Drop existing constraints if re-running
        existing_cons = list(ampl.getConstraints().keys())
        if constr_lower in existing_cons:
         ampl.eval(f"drop {constr_lower};")
        if constr_upper in existing_cons:
         ampl.eval(f"drop {constr_upper};")
    
            # Add new constraints
        ampl.eval(f"subject to {constr_lower}: {var_name} >= {lb};")
        ampl.eval(f"subject to {constr_upper}: {var_name} <= {ub};")
